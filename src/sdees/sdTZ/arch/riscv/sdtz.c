...
void sdtz_copy_args(struct vcpu *vcpu_dst, struct vcpu *vcpu_src,
        size_t num_args) {
    for (size_t i = 0; i < num_args; i++) {
        size_t regid = HYPCALL_IN_ARG_REG(i);
        vcpu_writereg(vcpu_dst, regid, vcpu_readreg(vcpu_src, regid));
    }
}

void sdtz_copy_args_call_done(struct vcpu *vcpu_dst, struct vcpu
...
static int64_t sdtz_smc_handler(struct vcpu* vcpu, uint64_t smc_fid) {
    ...
    if (calling_vcpu->vm->type == 0) { /* normal world */
            /* CROSSCON TODO: assumes call is for trusted OS */
            ret = sdtz_handler(vcpu, smc_fid);
    }else{
            ret = sdtz_handler(vcpu, smc_fid);
    }
