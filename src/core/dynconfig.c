...
static void dynconfig_adjust_vm(struct vm_config* vm_cfg, struct dynconfig* dynconfig,
    paddr_t load_addr)
{
    ...
    if ((vm_cfg->platform.devs = adjust_ptr(vm_cfg->platform.devs, (uintptr_t)dynconfig))) {
        for (size_t j = 0; j < vm_cfg->platform.dev_num; j++) {
            vm_cfg->platform.devs[j].interrupts =
                adjust_ptr(vm_cfg->platform.devs[j].interrupts, (uintptr_t)dynconfig);
        }
    }

    if ((vm_cfg->platform.ipcs = adjust_ptr(vm_cfg->platform.ipcs, (uintptr_t)dynconfig))) {
        for (size_t j = 0; j < vm_cfg->platform.ipc_num; j++) {
            vm_cfg->platform.ipcs[j].interrupts =
                adjust_ptr(vm_cfg->platform.ipcs[j].interrupts, (uintptr_t)dynconfig);
        }
    }
}
